...
test.describe('Connect Section', () => {
  test('Verify Contact Information Display', async ({ page }) => {
    ...
    await page.goto('https://studio-maell.vercel.app/connect');

    // 2. Verify heading "Excited about Design, Inclusion or Accessibility? Let's chat!" is displayed
    await expect(page.getByRole('heading', { name: 'Excited about Design, Inclusion or Accessibility? Let\'s chat!' })).toBeVisible();
    ...
    await expect(emailLink).toHaveAttribute('href', 'mailto:monsehopman@gmail.com');

    // 4. Verify LinkedIn profile link is visible: "linkedin.com/user/monsemaell"
    const linkedInLink = page.getByRole('link', { name: 'linkedin.com/user/monsemaell' });
    await expect(page.getByText('linkedin.com/user/monsemaell')).toBeVisible();

    // Verify LinkedIn link has correct URL
    await expect(linkedInLink).toHaveAttribute('href', 'https://linkedin.com/user/monsemaell');
